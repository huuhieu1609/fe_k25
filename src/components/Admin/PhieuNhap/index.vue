<template>
  <div class="row">
    <!-- 1. Danh Sách Phiếu Nhập (Toàn màn hình - 12) -->
    <div class="col-lg-12">
      <div class="card border-top border-0 border-4 border-primary shadow-sm mt-2">
        <div class="card-header d-flex justify-content-between align-items-center">
          ...
          <button class="btn btn-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#createModal">
            <i class="fa fa-plus me-2"></i>Thêm Mới Phiếu Nhập
          </button>
        </div>
        <div class="card-body">
              ...
              <tbody>
                <template v-for="(value, index) in list_phieu_nhap" :key="index">
                  <tr class="text-center">
                    ...
                    <td class="text-start">{{ value.ten_nha_cung_cap || "---" }}</td>
                    <td class="text-start">{{ value.ten_kho || "---" }}</td>
                    <td class="text-start">{{ value.ten_nhan_vien || "---" }}</td>
                    <td class="text-start">
                      {{
                        value.danh_sach_san_pham || value.ten_san_pham || "---"
                      }}
                    </td>

                    <td class="text-end">{{ formatCurrency(value.tong_tien_hang || 0) }}</td>
                    <td class="text-end">{{ formatCurrency(value.chiet_khau || 0) }}</td>
                    <td class="text-end">{{ formatCurrency(value.thue_vat || 0) }}</td>
                    <td class="text-end fw-bold text-primary">
                      {{ formatCurrency(value.tong_thanh_toan || 0) }}
                    </td>
                    <td class="text-end text-success">{{ formatCurrency(value.da_thanh_toan || 0) }}</td>
                    <td class="text-end text-danger fw-bold">{{ formatCurrency(value.con_no || 0) }}</td>
                    ...
                    <td class="text-nowrap text-center">
                      ...
                      <button class="btn btn-danger btn-sm shadow-sm" data-bs-toggle="modal"
                        @click="confirmXoa(value)" data-bs-target="#deleteModal">
  ...
  <!-- 2. Modal Thêm Mới Phiếu Nhập -->
  <div class="modal fade" id="createModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title fw-bold">Thêm Mới Phiếu Nhập</h5>
        ...
        <div class="modal-body">
          <form>
            <div class="row">
              ...
              <div class="col-md-4 mb-3">
                <label class="form-label fw-bold text-success">Tổng Tiền Hàng</label>
                <input v-model.number="them_phieu_nhap.tong_tien_hang" type="number" min="0"
              ...
              <div class="col-md-4 mb-3">
                <label class="form-label fw-bold text-primary">Tổng Thanh Toán</label>
                <input :value="calculateTotal(them_phieu_nhap)" type="number" class="form-control shadow-sm bg-light"
                  disabled />
              </div>
              <div class="col-md-4 mb-3">
              ...
              <div class="col-md-4 mb-3">
                <label class="form-label fw-bold text-danger">Còn Nợ</label>
                <input :value="calculateDebt(them_phieu_nhap)" type="number" class="form-control shadow-sm bg-light"
                  disabled />
              </div>
              <div class="col-md-12 mb-3">
                <label class="form-label fw-bold">Ghi Chú</label>
                <input v-model="them_phieu_nhap.ghi_chu" type="text" class="form-control"
        ...
        <div class="modal-footer bg-light">
          ...
          <button class="btn btn-primary px-4 shadow-sm" @click="addPhieuNhap()" data-bs-dismiss="modal">
            <i class="fa fa-save me-2"></i>Lưu Phiếu Nhập
          </button>
        </div>
      </div>
    </div>
        ...
        <div class="modal-body">
          <form>
            <div class="row">
              ...
              <div class="col-md-4 mb-3">
                <label class="form-label fw-bold font-weight-bold">Tổng Tiền Hàng</label>
                <input v-model.number="edit_phieu_nhap.tong_tien_hang" type="number" min="0"
                  class="form-control shadow-sm border-success" />
              </div>
              <div class="col-md-4 mb-3">
                <label class="form-label fw-bold">Chiết Khấu</label>
              ...
              <div class="col-md-4 mb-3">
                <label class="form-label fw-bold text-primary">Tổng Thanh Toán</label>
                <input :value="calculateTotal(edit_phieu_nhap)" type="number" class="form-control shadow-sm bg-light"
                  disabled />
              </div>
              <div class="col-md-4 mb-3">
                <label class="form-label fw-bold">Đã Thanh Toán</label>
              ...
              <div class="col-md-4 mb-3">
                <label class="form-label fw-bold text-danger">Còn Nợ</label>
                <input :value="calculateDebt(edit_phieu_nhap)" type="number" class="form-control shadow-sm bg-light"
                  disabled />
              </div>
              <div class="col-md-12 mb-3">
                <label class="form-label fw-bold">Ghi Chú</label>
                <input v-model="edit_phieu_nhap.ghi_chu" type="text" class="form-control shadow-sm"
                  placeholder="Ghi chú thêm..." />
              </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body text-center py-4">
          ...
          <h5 class="fw-bold fs-5">
            Bạn có chắc chắn muốn xóa phiếu nhập: <span class="text-danger">{{ xoa_phieu_nhap.ma_phieu }}</span>?
          </h5>
        ...
        <div class="modal-footer justify-content-center">
          ...
          <button type="button" class="btn btn-danger px-4 shadow-sm" @click="xoaPhieuNhap()" data-bs-dismiss="modal">
            Xác Nhận Xóa
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    openEdit(item) {
      ...
      axios
        ...
        .then((response) => {
          ...
          const details = response.data.chi_tiet || [];
          ...
          this.edit_phieu_nhap = {
            ...
            tong_tien_hang: Number(header.tong_tien_hang) || 0,
    ...
    openChiTiet(item) {
      ...
      axios
        ...
        .then((response) => {
          ...
          this.chi_tiet_list = response.data.chi_tiet || [];
        })
        .catch((error) => {
    ...
    calculateTotal(obj) {
      return (
        (Number(obj.tong_tien_hang) || 0) -
        (Number(obj.chiet_khau) || 0) +
        (Number(obj.thue_vat) || 0)
      );
    },
    calculateDebt(obj) {
      return (
        this.calculateTotal(obj) - (Number(obj.da_thanh_toan) || 0)
      );
    },
    confirmXoa(item) {
      this.xoa_phieu_nhap = Object.assign({}, item);
    },
    xoaPhieuNhap() {
      if (!this.xoa_phieu_nhap.id) {
        this.$toast?.error("Không tìm thấy ID phiếu cần xóa.");
        return;
      }
