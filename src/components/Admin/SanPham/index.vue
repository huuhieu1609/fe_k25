              ...
              <tbody>
                <template v-for="(value, index) in list_san_pham" :key="value.id">
                  <tr class="text-center">
                    <td>{{ index + 1 }}</td>
                    <td>
                      <img :src="value.hinh_anh" alt="Nho Mẫu Đơn" class="img-fluid rounded shadow-sm"
                    ...
                    <td class="text-center text-secondary">
                      {{ value.quan_ly_serial }}
                    </td>

                    <td>{{ value.thue_vat }}%</td>
                    <td class="text-end fw-bold">
                      {{ formatCurrency(value.gia_nhap_mac_dinh) }}
                    </td>
                    <td class="text-end fw-bold text-danger">
                      {{ formatCurrency(value.gia_ban_mac_dinh) }}
                    </td>
      ...
      axios
        ...
        .catch((error) => {
          ...
          if (
            ...
            this.$toast.error(
              "Đã xảy ra lỗi khi thay đổi trạng thái Sản Phẩm.",
            );
          }
        });
    },
